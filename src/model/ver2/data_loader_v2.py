...
class DataLoader(object):
    ...
    def next(self):
        ...
        if self.step < self.total_step:
            ...
            inp_tensor_anchor = Variable(torch.FloatTensor(inp_anchor), requires_grad=True)
            msk_tensor_anchor = Variable(torch.FloatTensor(msk_anchor), requires_grad=True)
            inp_tensor_pos = Variable(torch.FloatTensor(inp_pos), requires_grad=True)
            msk_tensor_pos = Variable(torch.FloatTensor(msk_pos), requires_grad=True)
            inp_tensor_neg = Variable(torch.FloatTensor(inp_neg), requires_grad=True)
            msk_tensor_neg = Variable(torch.FloatTensor(msk_neg), requires_grad=True)
