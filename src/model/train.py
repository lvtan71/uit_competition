...
def eval_model(model, validset_reader):
    model.eval()
    correct_pred = 0.0
    for inp_tensor_pos, msk_tensor_pos, inp_tensor_neg, msk_tensor_neg in validset_reader:
        prob_pos = model(inp_tensor_pos, msk_tensor_pos)
        prob_neg = model(inp_tensor_neg, msk_tensor_neg)
...
def train_model(model, args, trainset_reader, validset_reader):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for inp_tensor_pos, msk_tensor_pos, inp_tensor_neg, msk_tensor_neg in trainset_reader:
            ...
            label = torch.ones(score_pos.size()) # To use in MarginRankingLoss, assume the first elememt is grater than the second
            if args.cuda:
                label = label.cuda()
